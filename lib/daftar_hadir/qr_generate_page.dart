...
class _QrCodeGeneratorPageState extends State<QrCodeGeneratorPage> {
  ...
  Future<void> downloadQrCode() async {
    ...
    if (status.isGranted) {
      ...
      screenshotController.capture().then((image) async {
        if (image != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text('QR Code berhasil disimpan pada $fileName'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text('Membutuhkan izin penyimpanan untuk mendownload QR Code'),
          backgroundColor: Colors.red));
    }
  }
    ...
    return Scaffold(
        ...
        body: Column(
            ...
            children: [
              SizedBox(height: 20),
              Center(
                  child: Text('Absensi Tanggal: $todayDate',
                      style:
              ...
              if (qrData != null)
                ElevatedButton(
                    onPressed: downloadQrCode, child: Text('Download QR Code'))
            ]));
  }
}
